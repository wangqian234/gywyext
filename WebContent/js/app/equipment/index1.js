...
app.config([ '$routeProvider', function($routeProvider) {
	$routeProvider.when('/addCompany', {
		...
		templateUrl : '/gywyext/jsp/project/demo.html',
		controller : 'indexProController'
	})
	/*gaohaile tianjia*/ 
} ]);
...
app.factory('services', [ '$http', 'baseUrl', function($http, baseUrl) {
	...
	services.getTurn = function (data){
		return $http({
			method : 'post',
			url : baseUrl + 'systemProject/getTurn.do',
			data : data,
		})
	}
	//停止云台控制
	services.getStop = function (data){
		return $http({
			method : 'post',
			url : baseUrl + 'systemProject/getStop.do',
			data : data,
		})
	}
...
/*			function addProject(){
			...
			indexpro.selectCompanyById = function(companyId) {
				...
				services.selectCompanyById({
					...
					indexpro.company = data.company;
				});
			};
			// 修改公司信息
			indexpro.updateCompany = function() {
				var CoFormData = JSON.stringify(indexpro.company);
			...
			indexpro.selectProjectById = function(projectId) {
				...
				services.selectProjectById({
					...
					indexpro.project = data.project;
				});
			};
			// 修改项目信息
			indexpro.updateProject = function() {
				indexpro.project.company = indexpro.project.company.comp_id;
				var PrFormData = JSON.stringify(indexpro.project);
			...
			//gaogaogaogao
			//开始云台操作
			indexpro.getTurn = function (id){
				services.getTurn({
					turn_id : id
				}).success(function(data){
					indexpro.yunTurn = data;
					if(indexpro.yunTurn.result.code != "200"){
						alert(indexpro.yunTurn.result.msg);
					}
					console.log(JSON.stringify(indexpro.yunTurn));
				})
			}
			//停止云台操作
			indexpro.getStop = function (id){
				services.getStop({
					turn_id : id
				}).success(function(data){
					indexpro.yunStop = data;
					if(indexpro.yunTurn.result.code != "200"){
						alert(indexpro.yunStop.result.msg);
					}
					console.log(JSON.stringify(indexpro.yunStop));
				})
			}
			//haile	
			// 初始化页面信息
			function initData() {
				...
				if ($location.path().indexOf('/addProject') == 0){
					...
					services.selectProjectById({
						...
						indexpro.project = data.project;
						console.log(JSON.stringify(indexpro.project));
						indexpro.com = data.project.company;
						console.log(JSON.stringify(indexpro.com));
						indexpro.result = data.result;
						console.log(JSON.stringfy(indexpro.result));
					}) ;
					var proj_id=sessionStorage.getItem('proj_id');
					services.selectCompByProj({
						proj_id : proj_id
					}).success(function(data){
						indexpro.compy = data.company;
					})
